...
/**
 * React hook for managing a multi-line, Unicode-aware text buffer with editing, cursor movement, undo/redo, selection, clipboard, and external editor integration.
 *
 * Provides a comprehensive API for text manipulation, visual line wrapping, and cursor navigation in both logical and visual coordinates. Supports batch operations, word-wise editing, and viewport-based rendering for terminal-like environments.
 *
 * @returns A `TextBuffer` object exposing state and methods for editing, navigation, selection, clipboard, and integration with external editors.
 */
